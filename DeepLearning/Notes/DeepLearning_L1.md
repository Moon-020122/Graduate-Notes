...
下方将这些运算向量化，请注意W<sup>[1]</sup>在此处是一个[4,3]的矩阵，W<sup>[1]</sup><sub>X</sub>则是将其分解为产生的列向量，则要依照激活值a<sup>[0]</sup>来决定；下一层同理。
...
​	多个样本向量化即是进行列向量或行向量拼凑成矩阵，其中a和z都是向量，A和Z为向量拼凑称的大矩阵，列向量所组成的元素就是各个神经单元的数值，如A矩阵的左上角就为a<sup>[1]</sup><sup>(1)</sup><sub>1</sub>(上标：[第一层]（第一个样本）下标：第一个隐藏层神经元)；其余以此类推。
