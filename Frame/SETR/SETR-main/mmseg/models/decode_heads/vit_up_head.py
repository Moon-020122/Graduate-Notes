...
class VisionTransformerUpHead(BaseDecodeHead):
    ...
    def __init__(self, img_size=768, embed_dim=1024,
        ...
        self.num_upsampe_layer = num_upsampe_layer #上采样层
        self.conv3x3_conv1x1 = conv3x3_conv1x1 #卷积核大小
    ...
    def forward(self, x):
        ...
        if self.upsampling_method == 'bilinear': #双线性插值法上采样
            if x.dim() == 3:
                n, hw, c = x.shape
            ...
            if self.num_conv == 2:
                if self.num_upsampe_layer == 2:
                    ...
                    x = F.interpolate(
                        x, size=x.shape[-1]*4, mode='bilinear', align_corners=self.align_corners)#改变空间大小
                    x = self.conv_1(x)
                    x = F.interpolate(
