...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        head_dim = dim // num_heads #维度平分头数，使得每个头的维度为head_dim
        # NOTE scale factor was wrong in my original version, can set manually to be compat with prev weights
        self.scale = qk_scale or head_dim ** -0.5 #根号下d

        self.qkv = nn.Linear(dim, dim * 3, bias=qkv_bias) #全连接层
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = nn.Linear(dim, dim) #全连接层
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.attn = Attention(
            dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop)#自注意力机制
        ...
        self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim,
                       act_layer=act_layer, drop=drop)#多层感知层
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768):
        ...
        num_patches = (img_size[1] // patch_size[1]) * \
            (img_size[0] // patch_size[0]) #patch数量
        self.img_size = img_size
        self.patch_size = patch_size
        self.num_patches = num_patches

        self.proj = nn.Conv2d(in_chans, embed_dim,
                              kernel_size=patch_size, stride=patch_size) #调整channel数为embed_dim默认为768，在下方中输入为1024
...
class Conv_MLA(nn.Module):
    def __init__(self, in_channels=1024, mla_channels=256, norm_cfg=None):
        ...
        self.mla_p2_1x1 = nn.Sequential(nn.Conv2d(
            in_channels, mla_channels, 1, bias=False), build_norm_layer(norm_cfg, mla_channels)[1], nn.ReLU()) #此处的归一化层直接实例化
        ...
        self.mla_p2 = nn.Sequential(nn.Conv2d(mla_channels, mla_channels, 3, padding=1,
                                    bias=False), build_norm_layer(norm_cfg, mla_channels)[1], nn.ReLU()) #3*3卷积 填充1
    ...
    def forward(self, res2, res3, res4, res5):
        ...
        mla_p2_1x1 = self.mla_p2_1x1(res2) #reshape-conv

        mla_p4_plus = mla_p5_1x1 + mla_p4_1x1
        mla_p3_plus = mla_p4_plus + mla_p3_1x1
        mla_p2_plus = mla_p3_plus + mla_p2_1x1 #叠加特征图，生成图片，三次相加

        mla_p5 = self.mla_p5(mla_p5_1x1)
        mla_p4 = self.mla_p4(mla_p4_plus)
        mla_p3 = self.mla_p3(mla_p3_plus)
        mla_p2 = self.mla_p2(mla_p2_plus) #对照图，conv，conv-4X

        return mla_p2, mla_p3, mla_p4, mla_p5 #此处返回4个特征图
...
class VIT_MLA(nn.Module):
    ...
    def __init__(self, model_name='vit_large_patch16_384', img_size=384, patch_size=16, in_chans=3, embed_dim=1024, depth=24,
        ...
        self.img_size = img_size
        self.patch_size = patch_size #下采样倍率 16
        self.in_chans = in_chans#输入通道数
        self.embed_dim = embed_dim #嵌入维度 C
        self.depth = depth #Transformer的深度，图中对应24
        ...
        self.hybrid_backbone = hybrid_backbone #hybrid是CNN+Transformer
        self.norm_layer = norm_layer
        self.norm_cfg = norm_cfg
        self.pos_embed_interp = pos_embed_interp
        self.random_init = random_init
        self.align_corners = align_corners
        self.mla_channels = mla_channels#256
        self.mla_index = mla_index #分块0-6，7-12，13-18，19-24

        self.num_stages = self.depth #24
        self.out_indices = tuple(range(self.num_stages))#stage索引
        ...
        if self.hybrid_backbone is not None:
            ...
            self.patch_embed = PatchEmbed(
                img_size=self.img_size, patch_size=self.patch_size, in_chans=self.in_chans, embed_dim=self.embed_dim) #调整通道数为C
        self.num_patches = self.patch_embed.num_patches

        self.cls_token = nn.Parameter(torch.zeros(1, 1, self.embed_dim)) #添加一个类别token
        self.pos_embed = nn.Parameter(torch.zeros(
            1, self.num_patches + 1, self.embed_dim)) #位置编码
        self.pos_drop = nn.Dropout(p=self.drop_rate)#位置编码dropout

        dpr = [x.item() for x in torch.linspace(0, self.drop_path_rate,
                                                self.depth)]  # stochastic depth decay rule 递增dropout
        self.blocks = nn.ModuleList([
            ...
            for i in range(self.depth)])#!实例化Block 24个Transformer块
        #!实例化Conv_MLA
        self.mla = Conv_MLA(in_channels=self.embed_dim,
                            mla_channels=self.mla_channels, norm_cfg=self.norm_cfg) #1024，256（不太懂），None

        self.norm_0 = norm_layer(self.embed_dim)
        self.norm_1 = norm_layer(self.embed_dim)
        self.norm_2 = norm_layer(self.embed_dim)
        self.norm_3 = norm_layer(self.embed_dim) #下方归一化
    ...
    def forward(self, x):
        ...
        cls_tokens = self.cls_token.expand(B, -1, -1) #将cls_token的第一个维度扩展为B，以此添加到x中
        ...
        c6 = self.norm_0(outs[self.mla_index[0]]) #先归一化再输入进MLA
        ...
        return (p6, p12, p18, p24) #返回MLA的4个特征图
