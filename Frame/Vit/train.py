...
LEARNING_RATE = 1e-4
ADAM_WEIGHT_DECAY = 0
ADAM_BETAS = (0.9, 0.999) #优化器参数

train_dataloader, val_dataloader, test_dataloader = get_loaders(TRAIN_DF_DIR, TEST_DF_DIR, SUBMISSION_DF_DIR, BATCH_SIZE)#载入数据
...
optimizer = optim.Adam(model.parameters(), betas=ADAM_BETAS, lr=LEARNING_RATE, weight_decay=ADAM_WEIGHT_DECAY)#优化器参数设置

start = timeit.default_timer()

for epoch in tqdm(range(EPOCHS), position=0, leave=True): #显示进度条
    model.train()
    train_labels = []
    train_preds = []
